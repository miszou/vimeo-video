...
* Let TheGem Page Options control the surrounding content layout on default video pages
...
Yes. The single video page also works with classic themes through the plugin's fallback template. The fallback renders the Vimeo player and related videos through dynamic blocks. With TheGem, the plugin preserves TheGem's single-page wrapper for default video pages so Page Options > Content Layout settings can control the surrounding layout. Video posts support the editor Template selector, so you can choose a theme/page template instead of the plugin layout when your theme provides one. When TheGem is active, published TheGem Template Builder templates appear as selectable "TheGem:" options.

= How can I override the single video page layout? =

Edit a video post and use the Template selector to choose a theme/page template or a TheGem Template Builder template. In TheGem, you can also use Page Options > Content Layout to override the built-in content layout, template-builder layout, padding, and sidebar around the default video content. To apply a template to multiple videos, select videos on the Videos admin list, choose a "Set template:" bulk action, and click Apply. Theme developers can also add `single-mfvv_video.php` to the active theme or child theme to override all video detail pages.
